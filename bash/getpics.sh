...
if [[ `wget -q -S  --spider -O ~/public_html/pics/pics.tgz https://zonzorp.net/pics.tgz 2>&1 | grep 'HTTP/1.1 200 OK'` ]];
    then
        wget -q -O ~/public_html/pics/pics.tgz https://zonzorp.net/pics.tgz
        if [[ `tar zxvf  ~/public_html/pics/pics.tgz -C ~/public_html/pics 2>&1 |grep 'Error'` ]];
            then echo "Unable to uncompress ";
            else rm ~/public_html/pics/pics.tgz;
        fi;
    else echo "Download file not found";
fi
