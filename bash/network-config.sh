...
router_ip=$(ip route show | grep -i 'default via'| awk '{print $3 }')
router_name=$(grep $router_ip /etc/hosts | awk '{ print $2 }')
host_name=$(hostname)
network_interface=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
lan_address=$(ip a s $network_interface|awk '/inet /{gsub(/\/.*/,"");print $2}')
