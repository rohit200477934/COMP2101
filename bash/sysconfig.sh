...
function displayhelp {
    echo 'Arguments: ';
    echo '  --host                  get only hostname';
    echo '  --domain               get only domain';
    echo '  --ipconfig              get only ipconfig';
    echo '  --os                    get only os';
    echo '  --cpu                   get only cpu';
    echo '  --memory                get only memory';
    echo '  --disk                  get only disk';
    echo '  --printer               get only printer';
    echo '';
    echo 'Example usage:';
    echo '  sysconfig.sh --os';
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    -h|--help)
      displayhelp
      error-exit "help action need no further action"
      ;;
    --host)
      hostnamewanted=true
      partialreport=true
